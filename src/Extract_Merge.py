...
from google.cloud import bigquery  # keep this
import os, json
from google.oauth2 import service_account

def make_gcp_clients():
    """
    If GCP_SA_JSON is set (serverless/managed runs), build creds from it.
    Otherwise fall back to Application Default Credentials (local dev).
    Returns (storage_client, bigquery_client).
    """
    ...
    if sa_json:
        ...
        return (
            ...
            bigquery.Client(project=project_id, credentials=creds),
        )
    # Local fallback (uses GOOGLE_APPLICATION_CREDENTIALS path or gcloud auth)
...
def earthquake_pipeline(days_back: int = 1):
    ...
    for d in unique_dates:
        ...
        storage.Client().bucket(bucket_name).blob(path).upload_from_string(buf.getvalue())
